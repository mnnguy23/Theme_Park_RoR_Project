...
/*-------------------------------Triggers Functions-------------------------------*/
CREATE FUNCTION schedule_new_maintenance()
    RETURNS trigger AS 
$BODY$

BEGIN
 IF NEW.operational <> 'false' THEN
 INSERT INTO attraction_maintenance
 VALUES(now(), now() + interval '1 day', 1000, old.attraction_id, 1111115);
 END IF;
