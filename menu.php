...
function maintenanceReport($db, $isDevelopment) {
  $data = array();
  if($isDevelopment) {
    $results = pg_query($db ,"select E.e_name, A.name, M.maintenance_date, M.maintenance_cost, A.operational from public.employee as E, public.attraction as A, public.attraction_maintenance as M where M.e_id=E.employee_id and M.am_id=A.attraction_id;");
