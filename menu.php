...
function maintenanceReport($db, $isDevelopment) {
  ...
  if($isDevelopment) {
    ...
    $query = "SELECT E.employee_id, A.name, M.maintenance_date, M.maintenance_cost, A.operational FROM employee as E, attraction as A, attraction_maintenance as M WHERE M.e_id=E.employee_id and M.am_id=A.attraction_id;";
    ...
    while($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      $data[] = array($row["employee_id"], $row["name"], $row["maintenance_date"],$row["maintenance_cost"],  $isOperational);
    }
    $result->closeCursor();
  }  
  return $data;
